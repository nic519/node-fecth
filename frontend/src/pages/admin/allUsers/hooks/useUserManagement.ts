...
import { getAdminUserAll, getAdminUserDeleteByUid, postAdminUserCreate } from '@/generated/api-adapters';
...
export const useUserManagement = ({ superToken }: UseUserManagementProps): UseUserManagementReturn => {
	...
	const fetchUsers = async () => {
		try {
			...
			if (response.data.code !== 0) {
				setError(response.data.msg || '获取用户列表失败');
				setUsers([]);
				return;
			}
	...
	const handleAddUser = async () => {
		...
		try {
			...
			const response = await postAdminUserCreate(superToken, { uid: uid.trim(), config: userConfig });
			if (response.status === 201 && response.data.code === 0) {
				await fetchUsers(); // 重新加载用户列表
				alert(`用户 ${uid} 创建成功`);
			} else {
				alert('创建用户失败: ' + (response.data?.msg || '未知错误'));
			}
		} catch (error) {
