...
export function UserConfigPage({ userId }: UserConfigPageProps) {
  ...
  const loadConfig = async () => {
    try {
      setLoading(true);
      const response = await userConfigApi.get(userId, token);
      setConfig(response);
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      await userConfigApi.update(userId, newConfig, token);
              ...
              <div className="hidden md:block ml-10">
                <div className="flex items-baseline space-x-4">
                  <span className="text-sm text-gray-500">
                    用户ID: <span className="font-medium text-gray-900">{userId}</span>
                  </span>
                  <div className="flex items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 ${
                  saveSuccess ? 'bg-green-600 hover:bg-green-700 focus:ring-green-500' : 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500'
                }`}
              >
                {saving && (
                ...
                {!saving && !saveSuccess && (
                  <svg className="h-4 w-4 mr-2 transition-opacity duration-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
